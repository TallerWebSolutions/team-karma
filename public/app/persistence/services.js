...
angular.module('persistence')
  ...
  /**
   * Persistence responsible provider.
   */
  .provider('persistence', function() {

    // Define base connection.
    var _connection = {
      url: 'https://dump.firebaseio.com'
    };

    /**
     * Defines a connection to be used by the persistence.
     */
    this.setConnection = function(connection) { _connection = connection; };

    /**
     * Provider.
     */
    this.$get = function($firebase, $firebaseAuth, Util) {
      ...
      var persistence = {
        ...
        getReference: function(persistenceMap) {
          ...
          var url = _connection.url + (persistenceMap ? '/' + persistenceMap : '');
